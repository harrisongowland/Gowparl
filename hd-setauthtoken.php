...
if ($res->num_rows > 0){
  while ($row = $res->fetch_assoc()){
    if (password_verify($_POST['apiKey'], $row['identifier'])){
      $addAuthToken = $mysqli->prepare('UPDATE websiteData SET authToken = ?, guildID = ?');
      $addAuthToken->bind_param('ss', $_POST['authToken'], $_POST['guildID']);
  ...
  echo 'ERROR: no API key found. Not set up.';
}
